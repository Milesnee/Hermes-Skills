...
class TestInjectionLogic:
    ...
    def _simulate_injection(self, extra_raw: str, l1_passed: list[str],
        ...
        normalized = parsed  # 跳过 _normalize_symbols（mock）
        ...
        # 哨兵追踪
        trading_agents_extra_l4_triggers = {}
        trading_agents_extra_watch = []
        if trading_agents_extra_in_l1:
            ta_hit_set = set()
            for key, hits in bt.items():
                for code, score in hits:
                    if code in set(trading_agents_extra_in_l1):
                        ta_hit_set.add(code)
    ...
    def test_injection_into_l2_bypass(self):
        ...
        result = self._simulate_injection(
            ...
            ["000001"],  # 只有 000001 通过 L2
        )
        # 600519 被 L2 拒绝 → 注入到 l2_bypass_in_sector
        assert "600519" in result["l2_bypass_in_sector"]
        # 000001 通过 L2 → 不进 bypass
    ...
    def test_l4_triggers_promoted_to_bypass_pool(self):
        ...
        # 600519 有 SOS 触发 → 进 bypass_pool
        assert "600519" in result["l2_bypass_pool"]
        # 000001 无触发 → 不进
        assert "000001" not in result["l2_bypass_pool"]
        # 300750 虽有触发但不在精选池 → 哨兵追踪逻辑不会额外添加它
        # （但 bypass_pool 里可能因为 bypass_triggers 本身有它）
...
class TestScoreBonus:
    ...
    def test_does_not_override_existing_score(self):
        ...
        code_to_total_score = {"600519": 28.5}
        code_to_reasons = {"600519": ["SOS"]}
        code_to_trigger_keys = {"600519": ["SOS"]}
